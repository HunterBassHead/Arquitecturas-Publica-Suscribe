...
class XiaomiMyBand:
    ...
    def publish(self):
        ...
        message = {}
        message['body_x_position'] = self.simulate_x_position()
        message['body_y_position'] = self.simulate_y_position()
        message['body_z_position'] = self.simulate_z_position()
        message['id'] = str(self.id)
        message['datetime'] = self.simulate_datetime()
        message['producer'] = self.producer
        message['model'] = self.model
        ...
        print('[x] publicando valor del acelerometro...')
        self.draw_progress_bar(2)
        channel.basic_publish(exchange='', routing_key='body_acelerometro', body=str(message), properties=pika.BasicProperties(
            delivery_mode=2,))  # Se realiza la publicación del mensaje en el Distribuidor de Mensajes
        connection.close()  # Se cierra la conexión
        print('[x] valor publicado!')
        print('')
        ...
        print('[x] publicando valor del temporizador...')
        self.draw_progress_bar(2)
        channel.basic_publish(exchange='', routing_key='body_temporizador', body=str(message), properties=pika.BasicProperties(
            delivery_mode=2,))  # Se realiza la publicación del mensaje en el Distribuidor de Mensajes
        connection.close()  # Se cierra la conexión
        print('[x] valor publicado!')
        print('')
